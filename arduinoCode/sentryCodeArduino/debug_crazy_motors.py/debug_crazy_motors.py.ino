...
public:
  int currentTriggerCommand; // Public variable to store the current trigger position
  ...
  TriggerAndESC(int triggerPin, int escPin, int idlePwm, int spinPwm, int forwardPos, int backPos, int motorSpeedUpMs, int triggerMoveForwardMs)
    : flywheelIdlePwm(idlePwm), flywheelSpinPwm(spinPwm), 
      ...
      currentState(SHOT_STATE_IDLE), shootCommandActive(false), currentTriggerCommand(backPos) {
  ...
  void setStateToSpinFlywheel() {
    ...
    currentTriggerCommand = triggerBackPos; // Update the current command
  ...
  void setStateToTriggerForward() {
    ...
    currentTriggerCommand = triggerForwardPos; // Update the current command
    stateEnterTimestamp = millis();
...
TriggerAndESC triggerAndESC(6, 9, 990, 1150, 90, 10, 1000, 700);
...
void loop() {
  ...
  // Debugging: Print current trigger servo command
  Serial.print("Trigger Servo Command: ");
  Serial.println(triggerAndESC.currentTriggerCommand);
